...
The system is very very very slow in interpreted mode, but there will be multiple levels of https://github.com/benrayfield/wikibinator106/blob/main/wikibinator106/spec/Evaler.java optimizations. It can do about 1 million lambdas per second, and to multiply 2 floats costs a few thousand lambdas so maybe 300 flops. Not gigaflops. Just flops. 0.0000003 teraflops. With https://github.com/benrayfield/lazycl/ optimization that will get up to about 100 gigaflops (on GPU) and https://en.wikipedia.org/wiki/Javassist about 1 gigaflop in CPU, so to be competitive with other number crunching systems I plan to make it about 300,000,000 times faster than it currently is (normal GPU speeds). Times the number of computers its run on at once. On 1 million computers (most of which have a person using the system, other computers optionally in clouds), at 10^11 flops (float multiplies per second), at 3000 lambdas per flop, that would be 300,000,000,000,000,000,000 flops aka 300 petaflops shared among 1 million simultaneous users each providing part of that compute power toward their shared goals.
